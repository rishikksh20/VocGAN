...
class ModifiedGenerator(nn.Module):
    def __init__(self, mel_channel, n_residual_layers, ratios=[4, 4, 2, 2, 2, 2], mult=256, out_band=1):
        ...
        r = ratios[0]
        self.upsample_1 = Upsample(mult, r)
        self.res_stack_1 = nn.Sequential(*[ResStack(mult, dilation=3 ** j) for j in range(n_residual_layers)])

        r = ratios[1]
        mult = mult // 2
        self.upsample_2 = Upsample(mult, r)
        self.res_stack_2 = nn.Sequential(*[ResStack(mult, dilation=3 ** j) for j in range(n_residual_layers)])

        r = ratios[2]
        mult = mult // 2
        self.upsample_3 = Upsample(mult, r)
        ...
        r = ratios[3]
        mult = mult // 2
        self.upsample_4 = Upsample(mult, r)
        ...
        r = ratios[4]
        mult = mult // 2
        self.upsample_5 = Upsample(mult, r)
        ...
        r = ratios[5]
        mult = mult // 2
        self.upsample_6 = Upsample(mult, r)
...
if __name__ == '__main__':
    ...
    model = ModifiedGenerator(80, 4)

    x = torch.randn(3, 80, 10)  # (B, channels, T).
    print(x.shape)

    out1 = model(x)  # (B, 1, T ** prod(upsample_scales)
    print(out1.shape)
