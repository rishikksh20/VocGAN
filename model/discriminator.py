...
class JCU_Discriminator(nn.Module):
    def __init__(self):
        ...
        self.mel_conv1 = nn.Conv1d(80, 128, kernel_size=2, stride=1)
        self.mel_conv2 = nn.Conv1d(128, 128, kernel_size=5, stride=1, padding=2)
        self.mel_conv3 = nn.Conv1d(128, 1, kernel_size=3, stride=1, padding=1)

        self.x_conv1 = nn.Conv1d(1, 16, kernel_size=15, stride=1)
        self.x_conv2 = nn.Conv1d(16, 64, kernel_size=41, stride=4, padding=4 * 5,
                    groups=16 // 4)
        self.x_conv3 = nn.Conv1d(64, 128, kernel_size=21, stride=2)
        self.x_conv4 = nn.Conv1d(128, 128, kernel_size=5, stride=1, padding=2)
        self.x_conv5 = nn.Conv1d(128, 1, kernel_size=3, stride=1, padding=1)

    def forward(self, x, mel):
        out = self.mel_conv1(mel)
        out1 = self.x_conv1(x)
        out1 = self.x_conv2(out1)
        out1 = self.x_conv3(out1)
        out = torch.cat([out, out1], dim=2)
        out = self.mel_conv2(out)
        cond_out = self.mel_conv3(out)
        out1 = self.x_conv4(out1)
        uncond_out = self.x_conv5(out1)
